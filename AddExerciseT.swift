...
struct AddExerciseT: View {
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                NavigationLink(destination: ExerciseDetailViewT(workout: $workout, workoutDay: workoutDay, exercises: $exercises, selectedExercise: $selectedExercise, db: db, sharedCollectionName: sharedCollectionName), isActive: $showingExerciseDetailView) {
                    EmptyView()
                }
                .opacity(0.0)
    ...
    private func fetchExercisesForWorkout() {
        db.collection("SharedCollection") // Use the SharedCollection
            .document(sharedCollectionName) // Use the sharedCollectionName
...
struct ExerciseDetailViewT: View {
    ...
    init(workout: Binding<Workout>, workoutDay: WorkoutDay, exercises: Binding<[Exercise]>, selectedExercise: Binding<Exercise?>, db: Firestore, sharedCollectionName: String) {
    ...
    private func updateExerciseInFirestore(exercise: Exercise) {
        ...
        let exerciseRef = db.collection("SharedCollection") // Use the "SharedCollection"
            .document(sharedCollectionName) // Use the sharedCollectionName
            .collection("WorkoutDays") // Add your hierarchy as needed
            .document(workoutDay.id)
            .collection("Workouts")
            .document(workout.id)
            .collection("Exercises")
    ...
    private func saveExerciseToFirestore(exercise: Exercise) {
        ...
        let exercisesCollectionRef = db.collection("SharedCollection") // Use the "SharedCollection"
            .document(sharedCollectionName) // Use the sharedCollectionName
            .collection("WorkoutDays") // Add your hierarchy as needed
            .document(workoutDay.id)
            .collection("Workouts")
